...
from GWASImpl import GWAS
impl_GWAS = GWAS(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'GWAS'
        ...
        self.rpc_service.add(impl_GWAS.prepare_variation,
                             name='GWAS.prepare_variation',
                             types=[dict])
        self.method_authentication['GWAS.prepare_variation'] = 'required'
        self.rpc_service.add(impl_GWAS.calculate_kinship_matrix,
                             name='GWAS.calculate_kinship_matrix',
                             types=[dict])
        self.method_authentication['GWAS.calculate_kinship_matrix'] = 'required'
        self.rpc_service.add(impl_GWAS.run_gwas,
                             name='GWAS.run_gwas',
                             types=[dict])
        self.method_authentication['GWAS.run_gwas'] = 'required'
        self.rpc_service.add(impl_GWAS.variations_to_genes,
                             name='GWAS.variations_to_genes',
                             types=[dict])
        self.method_authentication['GWAS.variations_to_genes'] = 'required'
        self.rpc_service.add(impl_GWAS.genelist_to_networks,
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "GWAS but no authentication header was passed"
                            raise err
